function initHtml() {
	let html = "\
    <div id='lotus-tool-edit' class='mdui-theme-layout-dark mdui-shadow-7 mdui-color-grey-900'>\
        <div class='lotus-tool-edit-header'>\
            <svg class='lotus-tool-edit-icon' viewBox='0 0 1024 1024' version='1.1' xmlns='http://www.w3.org/2000/svg' p-id='1115' width='28' height='28'>\
        ...
        <div id='lotus-tool-edit-content' class='mdui-table-fluid'>\
            <div class='mdui-textfield'>\
                <input id='lotus-tool-edit-input' class='mdui-textfield-input' type='text' placeholder='请输入卖家编码' />\
                <button id='lotus-tool-edit-button' class='mdui-btn mdui-color-grey-800 mdui-btn-block'>批量修改</button>\
            </div>\
        </div>\
    </div>";

	$("body").prepend(html);
	$("#lotus-tool-edit-button").click(inputCode);
}

function inputCode() {
	$(".table-main").scrollLeft(9999);

	let code = $("#lotus-tool-edit-input").val(),
		colspan = 0;

	$(".table-node th").each((index, element) => {
		colspan += parseInt($(element).attr("colspan"));
		if (element.innerText.indexOf("商品代码") != -1) return false;
	});

	$(".wuic-table-main tr td:nth-child(" + colspan + ") input").each((index, element) => {
		element.value = code;
		element.dispatchEvent(new Event("input"));
	});

	mdui.alert("本商品 卖方商品代码 填充完毕! 保存后请详细检查, 以防漏掉!");
}
...
function initTool() {
	...
	let temp;
	setInterval(() => {
		let show = $(".wing-web-component div[data-wuic-partial=widget]").length > 0;
		if (temp != show) {
			temp = show;
			if (show) {
				$("#lotus-tool-edit").show();
				$(".table-main").scrollLeft(9999);
			} else {
				$("#lotus-tool-edit").hide();
			}
		}
	}, 500);
}
