function initHtml() {
	$("#productList > li").each((index, element) => {
		let obj = $(element).find(".search-product-link");

		otherSellerJson(obj.attr("data-product-id"), obj.attr("data-item-id"), obj.attr("data-vendor-item-id"), (success, data) => {
			if (success) {
				let count = data.totalCount > 0 ? data.totalCount : "";
				let html = "\
				<div id='lotus-tool-info' class='mdui-theme-layout-dark mdui-shadow-7 mdui-color-grey-900 lotus-tool-info-header" + (count == "" ? " lotus-tool-info-whith" : "") + "'>\
...
function initTool() {
	selectWait("#productList > li", initHtml, 100, 500);
}
